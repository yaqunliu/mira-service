...
async def resolve_resource_reference(
    ...
    Args:
        ...
        user_reference: 用户的原始描述（如"分镜5"、"幽影"、"第一个分镜"）
        
    Returns:
        匹配结果，包含 matched_resources 和 confidence
    """
    ...
    try:
        ...
        # 降级为简单匹配
        return _fallback_match(target, user_reference, all_resources)
...
```"""
    ...
    for i, r in enumerate(all_resources[:30], 1):  # 最多30个
        if target == "shot":
            ...
            desc = f"ID:{r.get('shot_id') or r.get('id')}, 编号:{shot_num}, 描述:{r.get('description', '')[:40]}"
        elif target == "character":
            desc = f"ID:{r.get('character_id') or r.get('id')}, 名称:{r.get('name')}"
        elif target == "scene":
            desc = f"ID:{r.get('scene_id') or r.get('id')}, 标题:{r.get('title')}"
        else:
            desc = f"ID:{r.get('id')}"
