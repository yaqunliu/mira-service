from sqlalchemy import Column, DateTime, ForeignKey, Integer, String, Text, Float, Enum as SQLEnum
...
class WorkflowMode(str, enum.Enum):
    """工作流模式枚举"""
    TRADITIONAL = "traditional"  # 传统手动工作流（Celery 任务队列）
    AGENT = "agent"             # Agent 智能协助工作流（LangGraph）


class Creation(Base):
    """创作模型 - 支持传统手动工作流和 Agent 智能协助工作流"""
    ...
    workflow_mode = Column(
        SQLEnum(WorkflowMode, name="workflow_mode_enum", create_type=False),
        default=WorkflowMode.TRADITIONAL,
        nullable=False,
        index=True,
        comment="工作流模式：traditional=传统手动工作流（Celery），agent=Agent智能协助工作流（LangGraph）"
    )
    agent_session_id = Column(
        String(100),
        nullable=True,
        index=True,
        comment="关联的 Agent 会话 ID（仅 workflow_mode=agent 时有效）"
    )
    # ====================================================================
    ...
    agent_session = relationship(
        ...
        back_populates="creation",
        uselist=False,
        foreign_keys="AgentSession.creation_id"
    )

    @property
    def is_agent_mode(self) -> bool:
        """判断是否为 Agent 工作流模式"""
        return self.workflow_mode == WorkflowMode.AGENT

    @property
    def is_traditional_mode(self) -> bool:
        """判断是否为传统工作流模式"""
        return self.workflow_mode == WorkflowMode.TRADITIONAL
