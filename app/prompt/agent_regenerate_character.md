...
**当 target_id 未指定时，必须从用户消息中提取角色名并查找：**

1. **从用户消息中提取角色名**（如"张三"、"阿九"、"主角"）
2. **调用 query_characters 获取角色列表**
3. **在返回的列表中查找匹配的角色名**（模糊匹配）
   - 比较角色名的相似度
   - 找到最匹配的角色，获取其 character_id
4. **调用 query_single_character 获取详细信息**

**重要**：
- 不要只调用 query_characters 就停止！
- 必须在返回的列表中找到匹配的角色，然后继续执行后续步骤
- 如果找不到匹配的角色，告知用户"未找到角色 XXX，请确认角色名是否正确"
...
2. 调用 `query_generation_tasks_status` 查询任务状态（阻塞等待完成）
   ...
   - 参数 timeout: 1200, poll_interval: 2.0
3. 根据返回结果汇报成功或失败
...
- "重新生成阿九的图片" → name="阿九"
- "给主角生图" → name="主角"
- "重新生成角色张磊的提示词" → name="张磊"（注意："角色"后面直接跟名字）
- "生成角色李明的图像" → name="李明"
- "重新生成所有角色" → 遍历所有角色
- 如果无法提取，调用 query_characters 获取列表让用户确认

**重要**：当用户说"角色XXX"时，"角色"是类型指示词，后面的"XXX"才是角色名！
