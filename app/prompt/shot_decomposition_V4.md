...
6. **出镜元素 (appearance_elements)**：列出分镜中**实际出现**的关键物品、工具或道具
   ...
   - **关键一致性要求**：必须检查上一分镜的`出镜元素`。如果人物在上一分镜中手持某物（如手机、行李箱），且当前分镜剧情连续，该物品**必须**继续出现在当前分镜的`出镜元素`中，除非剧情明确描述了放下或遗失的动作。
...
9. **简要剧情**：精简描述分镜的剧情内容（30-50字）
   ...
   - 结合**出镜元素**，确保剧情描述中体现了物品的状态（如：手里紧紧握着手机）。
...
6. **连贯性与一致性（硬性要求）**：
   - **相邻分镜的物品状态必须保持连续**。
   - 例如：分镜1中人物拿着行李箱，分镜2如果是紧接的连续动作，人物必须依然拿着行李箱，不允许凭空消失。
...
```json
{
  "分镜列表": [
    ...
    },
    {
      ...
      "分镜时长": 5,
      "简要剧情": "近景居中：林小雨手里紧紧攥着那张羊皮地图，举起电话，神情激动地向教授汇报发现。"
    }
  ]
}
```
...
[
  {
    ...
    "出镜角色": ["张伟-青年"],
    "出镜元素": ["红色行李箱", "手机"],
    "简要剧情": "近景居中：张伟左手拖着红色行李箱，右手拿着手机看导航，焦急地环顾四周。",
    "分镜时长": 5
  },
  {
    "分镜编号": "4-2",
    "出镜角色": ["张伟-青年"],
    "出镜元素": ["红色行李箱", "手机"],
    "简要剧情": "近景居中：张伟对着手机大声争辩，手里的行李箱依然紧紧攥着，表情愤怒。",
    "分镜时长": 6
  }
]
```
*解析：分镜4-1中出现了行李箱和手机，分镜4-2剧情连续，张伟依然拿着这两个物品，因此`出镜元素`中必须明确包含它们。*
...
// 上一分镜：人物手持鲜花
❌ "出镜元素": [],
❌ "简要剧情": "近景居中：他深情地看着对方，准备表白。"
```
*修正：虽然是连续动作，但`出镜元素`漏掉了鲜花。应加上 ["鲜花"]，剧情描述也应提及“捧着鲜花”。*
