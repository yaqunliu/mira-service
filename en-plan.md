...
## Phase 3.8 — 分镜拆解英文化 + character_id 泄漏修复

> **状态：已实现**（2026-09-02）。未验证项：端到端未跑（本地无 DB/Celery），
> `py_compile` 通过，`strip_leaked_character_ids` 已用 7 组 case 单测（含 3 组误伤场景）。

场景英文化（3.7）之后，分镜列表仍是中文。但截图暴露的**第一个问题不是语言**：

```
#1  近景居中: 10抱着一摞新课本站在教室门口，微微喘气。
              ↑ 这个 10 是 character_id
```

### 3.8.1 🔥 根因：id 引用规则被过度泛化到叙述文本

3.5 那轮把角色引用改成纯 ID 后，`shot_decomposition_V3.md` 里反复强调
「**禁止输出角色名，必须用 id**」（`:45,47,63,68` 四处）。但这条规则的适用范围只有三个字段：
`on_screen_character_ids`、`voice_character_ids`、`narration[].character_id`。

`description`（原 `简要剧情`）是**给人读的自然语言叙述，直接显示在前端列表上**，
必须写角色名。而原 prompt 的 6 处 `description` 示例用的全是中文角色名，
没有一处演示「id 字段 + 名字叙述字段」如何并存 —— LLM 于是把禁令泛化，写出了裸 id。

**这个 bug 独立于英文化**：中文小说现在同样会踩，是 3.5 引入的回归。

修复分两层：

| 层 | 位置 | 做法 |
|---|---|---|
| Prompt（主） | `shot_decomposition_V3.md:79-88` | `description` 条目下显式说明：必须写角色名，禁止写 id；列出 id 规则只适用的三个字段 |
| Prompt（主） | 新增错误示例 6 | 直接用截图里的 `"10 carries a stack of new textbooks..."` 当反例 |
| 代码（兜底） | `character_variants.py:strip_leaked_character_ids` | 落库前把小句开头的裸 id 换回角色名 |

**兜底函数的边界（重要）**：只替换**小句开头**的 id（字符串起始，或 `: ： , ， 。 . ! ？` 之后）。
最初写的是「独立词」匹配，单测立刻抓到两个误伤：

```
"waits 10 minutes"  → "waits Tao Wei minutes"   ❌ 时长被当成角色
"Shot 1-10 shows…"  → "Shot Lin Xiaoyu-Tao Wei" ❌ 分镜编号被拆掉
```

正则分不清「句首的 10 是角色」和「句中的 10 是时长/编号」。误改会造出比原 bug
更难发现的新错误，所以收窄到小句开头（正是实际观察到的错误形态），
其余残留**只打 warning 不改写**，交给 prompt 侧和人工复核。
...
| `分镜编号` | `shot_number` |
| `简要剧情` | `description` |
| `台词` | `narration`（**内部仍是 `{character_id, content}`，`角色`/`内容` 键留给 Phase 4**） |
| `分镜时长` | `duration` |
| `出镜元素` | `appearance_elements` |
| `运镜` / `音效` / `剧本正文` / `画面提示词` | `camera_movement` / `sound_effect` / `script_content` / `image_prompt` |

消费端：`creation_task.py:846,905,914-924`、`ai_client.py:899`。

顺带删掉了 prompt 末尾那句自相矛盾的说明
（「`出镜元素` 在代码中对应 `appearance_elements`，请确保 JSON key 使用 `出镜元素`」）。

`creation_task.py:914` 原先是 `画面内容 or 分镜内容 or 简要剧情` 三级回退，
但 prompt 从头到尾只输出 `简要剧情`，另外两个键从未出现过 —— 一并收敛成单一 `description`。

### 3.8.3 输出语言

- prompt 正文（任务要求、规则说明）**保留中文**——LLM 读中文指令没问题，本轮不动，减少改动面
- 输出值全部要求英文：新增「重要规则 6. 输出语言」硬性条款，逐字段列出
- 6 处 `description` 示例、5 条错误示例的值翻英文，新增错误示例 7（输出中文）
- 台词时长估算从「每秒 3.5 个汉字」改为「每秒 2.5 个英文单词」，
  `duration` 计算公式同步改（`字数÷3.5` → `单词数÷2.5`）
- `description` 字数约束从「30-50 字」改为「20-35 个英文单词」

### 3.8.4 死代码确认

`ai_client.py:767-768`（`gen_playbook_by_characters`）读 `场景拆解`/`分镜列表`，
是 V2 老链路。全仓 grep 无调用方，仅 `:475` 的废弃注释提到它。**本轮不动**，
留给后续统一清理——改死代码只会增加 diff 噪音。

### 3.8.5 验收

1. 传英文小说 → 分镜列表 `description` 全英文
2. **`description` 里没有裸数字角色引用**（截图那个 `10` 不再出现）
3. 日志中 `description 小句开头泄漏了 character_id` 为 0 条
   —— 出现即说明 prompt 侧修复未生效，兜底在替 prompt 擦屁股
4. 日志中 `仍有疑似 character_id 的裸数字` 为 0 条（有则需人工看一眼是否误报）
5. `narration` 落库形态仍是 `{"角色": ..., "内容": ..., "character_id": ...}`，前端零改动
6. 分镜时长之和落在 90-150 秒（验证英文单词计时公式没算崩）
7. `grep -rn "分镜列表\|简要剧情\|出镜元素" app/ --include=*.py` 仅剩注释

### 3.8.6 与 3.7 的发版关系

3.7 已上线的前提下，本轮**可独立发版**：分镜 prompt 与消费端在同一批改动内自洽，
不依赖 3.7 之外的任何新字段。但仍需 `celery_worker` 重启（分镜拆解跑在 Celery 里），
且发版前应排空队列——队列里 3.8 之前入队的任务用的是中文契约，
新代码读 `shots` 会拿到空列表，分镜直接不落库。
...
Phase 3.8 (分镜拆解)       │           ←── 独立，已完成；含 3.5 引入的 id 泄漏回归修复
   ↓                      │
